...
@Client.on_message(filters.command(["kurinji"]) & filters.group, group=1)
async def kurinji(bot: Bot, update):
    """
    A Funtion To Handle Incoming /kurinji Command TO Disconnect All Chats From A Group
    """
    chat_id=update.chat.id
    user_id = update.from_user.id if update.from_user else None
    global VERIFY
